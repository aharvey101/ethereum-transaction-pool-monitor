...
impl SandwichPoolIntegration {
    ...
    pub async fn get_sandwich_candidates(
        &self,
        min_liquidity_usd: f64,
    ) -> Result<Vec<LiquidityAnalysis>> {
        ...
        for protocol in &["UniswapV2", "UniswapV3", "SushiSwap"] {
            ...
            for pool in pools.into_iter().take(100) {
                // Limit to top 100 per protocol
    ...
    async fn analyze_pool_liquidity(&self, pool: &DexPool) -> Result<Option<LiquidityAnalysis>> {
        ...
        let price_impact_1_eth = self
            .calculate_price_impact(&pool_state, U256::from(10_u64.pow(18)))
            .await?;
        let price_impact_10_eth = self
            .calculate_price_impact(&pool_state, U256::from(10_u64.pow(19)))
            .await?;
    ...
    fn calculate_sandwich_score(
        ...
        if liquidity_usd >= 10_000_000.0 {
            score += 40;
        } else if liquidity_usd >= 5_000_000.0 {
            score += 35;
        ...
        if volume_24h_usd >= 5_000_000.0 {
            score += 30;
        } else if volume_24h_usd >= 1_000_000.0 {
            score += 25;
        } else if volume_24h_usd >= 500_000.0 {
            score += 20;
        } else if volume_24h_usd >= 100_000.0 {
            score += 15;
        } else if volume_24h_usd >= 50_000.0 {
            score += 10;
        }
        ...
        // 0.5% or less
        else if avg_impact <= 0.01 {
            score += 25;
        }
        // 1% or less
        else if avg_impact <= 0.02 {
            score += 20;
        }
        // 2% or less
        else if avg_impact <= 0.05 {
            score += 15;
        }
        // 5% or less
        else if avg_impact <= 0.1 {
            score += 10;
        } // 10% or less
    ...
    pub async fn analyze_victim_transaction(
        ...
        let pool = match self.pool_states.get(&target_pool) {
            ...
            None => {
                ...
                let pool_info = self
                    .pool_db
                    .get_pool(&target_pool.to_string(), 1)?
                    .ok_or_else(|| anyhow::anyhow!("Pool not found in database"))?;
                self.fetch_pool_state(target_pool, &pool_info.protocol)
                    .await?
            }
        };

        // Calculate optimal frontrun amount and expected profit
        let frontrun_amount = self
            .calculate_optimal_frontrun(&pool, &trade_details)
            .await?;
        let estimated_profit = self
            .estimate_sandwich_profit(&pool, &trade_details, frontrun_amount)
            .await?;

        // Calculate risk score
        let risk_score = self
            .calculate_risk_score(&pool, &trade_details, frontrun_amount)
            .await?;
    ...
    async fn calculate_optimal_frontrun(
        &self,
        _pool: &PoolState,
        trade: &TradeDetails,
    ) -> Result<U256> {
    ...
    async fn calculate_risk_score(
        ...
        let trade_percentage =
            (frontrun_amount.to::<u128>() as f64 / 10f64.powi(18)) / pool_liquidity_eth;

        if trade_percentage > 0.1 {
            risk += 30;
        }
        // >10% of pool
        else if trade_percentage > 0.05 {
            risk += 20;
        }
        // >5% of pool
        else if trade_percentage > 0.01 {
            risk += 10;
        } // >1% of pool
