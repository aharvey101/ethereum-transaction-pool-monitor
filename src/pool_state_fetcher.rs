...
impl PoolStateFetcher {
    ...
    pub async fn fetch_pool_state(
        ...
        match protocol {
            "UniswapV2" | "SushiSwap" => self.fetch_uniswap_v2_state(pool_address, protocol).await,
            "UniswapV3" => self.fetch_uniswap_v3_state(pool_address).await,
    ...
    async fn fetch_uniswap_v2_state(
        &self,
        pool_address: Address,
        protocol: &str,
    ) -> Result<PoolState> {
    ...
    pub async fn calculate_price_impact(
        ...
        match pool_state.protocol.as_str() {
            "UniswapV2" | "SushiSwap" => {
                self.calculate_v2_price_impact(pool_state, trade_amount, direction)
                    .await
            }
            "UniswapV3" => {
                self.calculate_v3_price_impact(pool_state, trade_amount, direction)
                    .await
            }
    ...
    async fn calculate_v2_price_impact(
        &self,
        pool_state: &PoolState,
        trade_amount: U256,
        direction: TradeDirection,
        ...
        let amount_out =
            (amount_in_with_fee * reserve_out_f64) / (reserve_in_f64 + amount_in_with_fee);
    ...
    async fn calculate_v3_price_impact(
        &self,
        pool_state: &PoolState,
        trade_amount: U256,
        ...
        let trade_percentage =
            trade_amount.to::<u128>() as f64 / total_liquidity.to::<u128>() as f64;
    ...
    pub async fn fetch_multiple_pools(
        &self,
