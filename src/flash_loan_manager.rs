...
impl FlashLoanManager {
    ...
    pub async fn initialize(&mut self, eth_client: &crate::eth_client::EthereumClient) -> Result<()> {
        ...
        for (token_addr, symbol) in major_tokens {
            ...
            let balance = self.get_flash_loan_available_amount(eth_client, token).await?;
            
            self.available_tokens.insert(token, balance);
            info!("   {} available for flash loan: {} tokens", symbol, balance);
        }
        
        info!("✅ Flash loan manager initialized with {} tokens", self.available_tokens.len());
        Ok(())
    }
    ...
    pub async fn execute_flash_loan_sandwich(
        ...
        debug!("   Expected profit: {} ETH", request.expected_profit.to::<u64>() as f64 / 1e18);
        
        // Validate flash loan request
        self.validate_flash_loan_request(&request)?;
        
        // Check if we have sufficient flash loan capacity
        let available = self.available_tokens.get(&request.token).unwrap_or(&U256::ZERO);
        if request.amount > *available {
            return Ok(FlashLoanResult {
                ...
                error_message: Some(format!("Insufficient flash loan capacity: need {}, have {}", 
        ...
        let execution_result = self.submit_flash_loan_transaction(
            eth_client,
            flash_loan_calldata,
        ...
        if execution_result.success {
            info!("✅ Flash loan sandwich successful: {} ETH profit in {}ms", 
                execution_result.profit_wei.to::<u64>() as f64 / 1e18,
                execution_time
            );
        } else {
            warn!("❌ Flash loan sandwich failed: {}", 
    ...
    async fn get_flash_loan_available_amount(
        ...
        match token {
            _ if token == self.weth_address => Ok(U256::from(10000u64) * U256::from(10u64).pow(U256::from(18))), // 10,000 WETH
    ...
    fn validate_flash_loan_request(&self, request: &FlashLoanRequest) -> Result<()> {
        ...
        if request.expected_profit < U256::from(1000000000000000u64) { // 0.001 ETH minimum
    ...
    pub fn get_available_capacity(&self, token: Address) -> U256 {
        self.available_tokens.get(&token).copied().unwrap_or(U256::ZERO)
    }
    ...
    pub fn set_execution_contract(&mut self, contract_address: Address) {
        ...
        info!("📝 Set sandwich execution contract: 0x{:x}", contract_address);
    }
...
mod tests {
    ...
    fn test_flash_loan_manager_creation() {
        ...
        assert_eq!(manager.vault_address, BALANCER_VAULT_ADDRESS.parse().unwrap());
        assert_eq!(manager.weth_address, WETH_ADDRESS.parse().unwrap());
    }
    ...
    fn test_estimate_flash_loan_amount() {
        ...
        let required_amount = estimate_flash_loan_amount(&sandwich_data, WETH_ADDRESS.parse().unwrap());
